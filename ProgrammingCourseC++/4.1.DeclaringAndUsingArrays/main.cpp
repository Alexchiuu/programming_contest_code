...
int main(){
    ...
    std::cout << std::endl;
    std::cout << "Reading with a loop(junk data) : " << std::endl;
    ...
    //Print the data out
    std::cout << std::endl;
    std::cout << "Reading with a loop(Initialized) : " << std::endl;
    for( size_t i {0} ; i < 10 ; ++i){
        std::cout << "scores [" << i << "] : " << scores[i] << std::endl;
    }
    ...
    // Read only arrays  
    const int birds[] {10,12,15,11,18,17,23,56}; //Can't modifile const data
    //birds[2] = 8;
